...
- Bancos, caixas econômicas, cooperativas de crédito, financeiras públicas
      ...
      res.on("end", () => {
        try {
          ...
          if (parsed?.error?.type === "rate_limit_error") {
            ...
            if (espera) {
              setTimeout(() => callAnthropic(query, qtd, filtro, ordem, existingCnpjs, tentativa + 1).then(resolve).catch(reject), espera);
            } else {
...
module.exports = async (req, res) => {
  ...
  const { query, qtd, filtro, ordem, existingCnpjs = [] } = req.body;
  if (!query) { res.status(400).json({ error: "Campo query obrigatório" }); return; }

  try {
    const apiResponse = await callAnthropic(query, qtd || 5, filtro || "todos", ordem || "relevancia", existingCnpjs);
