    ...
    req.on("end", async () => {
      try {
        ...
        if (!query) {
          ...
          res.end(JSON.stringify({ error: "Campo query obrigatório" }));
          return;
        }

        console.log(`\n[Buscando] "${query}" | qtd=${qtd} existentes=${existingCnpjs.length}`);
        ...
        if (apiResponse.type === "error" || apiResponse.error) {
          ...
          console.error("[API erro]", msg);
          res.writeHead(200, { "Content-Type": "application/json; charset=utf-8" });
          res.end(JSON.stringify({ query, total: 0, empresas: [], obs: msg }));
          return;
        }

        const textBlock = apiResponse.content?.find(b => b.type === "text");
        const raw = textBlock?.text || "";
        console.log("[stop]", apiResponse.stop_reason, "| texto:", raw.substring(0, 150));
        ...
        if (Array.isArray(result.empresas)) {
          ...
          // Palavras que indicam entidade excluída
          const excluir = [
          ...
          result.empresas = result.empresas.filter(e => {
            ...
            if (excluir.some(rx => rx.test(nome))) {
              console.log(`[Excluído] ${e.nome}`);
              return false;
            }
        ...
        console.log(`[OK] ${result.total} empresas confirmadas`);
        res.writeHead(200, { "Content-Type": "application/json; charset=utf-8" });
...
server.listen(PORT, () => {
  ...
  console.log(`   API KEY: ${ANTHROPIC_API_KEY !== "SUA_CHAVE_AQUI" ? "✓ configurada" : "⚠ não configurada"}\n`);
});
